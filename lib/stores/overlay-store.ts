...
export function clampOverlayPreferences(value: OverlayPreferences): OverlayPreferences {
    return {
        ...value,
...
export function mergeOverlayRuntimePreferences(
    preferences: OverlayPreferences,
    ...
    return clampOverlayPreferences({
        ...
        // Keep desired capture-shield state separate from effective runtime
        // support. Linux currently cannot enforce the shield, but the portable
        // preference should survive and become effective on Win/macOS.
...
export function overlayWindowConfig(preferences: OverlayPreferences): OverlayWindowConfig {
    const p = clampOverlayPreferences(preferences);
    return {
        width: p.width,
        height: p.height,
        x: p.x,
...
export const useOverlayStore = create<OverlayStoreState>()(
    persist(
        (set) => ({
            ...
            update: (patch) =>
                set((state) => ({
                    preferences: clampOverlayPreferences({ ...state.preferences, ...patch }),
                })),
            applyRuntime: (runtime) =>
                set((state) => {
                    const nextPreferences = mergeOverlayRuntimePreferences(state.preferences, runtime);
            ...
            reset: () => set({ preferences: DEFAULT_OVERLAY_PREFERENCES, runtime: null, lastError: null }),
        }),
        {
