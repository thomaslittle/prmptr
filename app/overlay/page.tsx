...
export default function OverlayPage() {
    ...
    return (
        <main
            className="h-screen w-screen overflow-hidden bg-transparent p-2 text-foreground select-none"
            style={{ fontSize: `${fontScale}rem` }}
        >
            <section
                className="flex h-full flex-col overflow-hidden rounded-xl border border-white/10 bg-background/95 shadow-2xl backdrop-blur-xl"
                style={{ opacity }}
            >
                <header
                    className="flex h-9 shrink-0 items-center gap-2 border-b border-border/70 px-3 cursor-move"
                    data-tauri-drag-region
                >
                    <Sparkle weight="fill" className="size-3 text-primary" />
                    <span className="text-[10px] font-semibold tracking-[0.18em] text-foreground/75">
                        PRMPTR
                    </span>
                    {content.isStreaming && (
                    ...
                    <div className="ml-auto flex items-center gap-1 text-muted-foreground/60">
                        {runtime?.captureProtected && (
                            <span title="Hidden from screen capture" className="inline-flex p-1">
                                <LockSimple className="size-3" />
                            </span>
                        )}
                        {runtime?.clickThrough ? (
                            <span title="Click-through enabled; use the main PRMPTR window or shortcut to disable it" className="inline-flex p-1">
                                <MouseSimple className="size-3" />
                            </span>
                        ) : (
                            <button
                                type="button"
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
                    {hasStreaming && (
                        <article className="mb-3 rounded-lg border border-primary/20 bg-primary/[0.06] p-3 text-[0.78em] leading-relaxed text-foreground">
                            <div className="mb-1 text-[0.65em] font-semibold uppercase tracking-[0.14em] text-primary/80">
                                Live response
                            </div>
                            <div className="overlay-markdown prose prose-invert prose-sm max-w-none leading-relaxed">
                                <ReactMarkdown>{content.currentResponse}</ReactMarkdown>
                            </div>
                        </article>
                    )}
                    ...
                    {completed.length > 0 ? (
                        <div className="space-y-2">
                            {completed.map((response, index) => (
                                <article
                                    key={response.id}
                                    className={`rounded-lg border border-border/70 bg-muted/20 p-3 text-[0.75em] leading-relaxed ${index === 0 && !hasStreaming ? "border-primary/20 bg-primary/[0.04]" : ""}`}
                                >
                                    ...
                                    <div className="overlay-markdown prose prose-invert prose-sm max-w-none leading-relaxed text-foreground/90">
