...
export default function Dashboard() {
    ...
    const LAYOUT_KEY = "prmptr-panel-layout";
    ...
    const feedItemsRef = useRef<FeedItem[]>([]); // updated below with mergedFeedItems
    ...
    const isLocalWhisper = (settings.transcriptionMode ?? "screenpipe") === "local-whisper";
    const isDirectDeepgram = (settings.transcriptionMode ?? "screenpipe") === "direct-deepgram";
    ...
    // Merge archived session items with live items (and local whisper items) so AI keeps full conversation context
    // Deduplicates by ID, keeps chronological order
    const mergedFeedItems = (() => {
        if (!isLiveFeed) return sessionFeedItems;
        const liveSources = (isLocalWhisper || isDirectDeepgram) ? [...localFeed.items] : [...feed.items];
        if (sessionFeedItems.length === 0) return liveSources;
        const seen = new Set(liveSources.map((i) => i.id));
        const archived = sessionFeedItems.filter((i) => !seen.has(i.id));
        if (archived.length === 0) return liveSources;
        return [...archived, ...liveSources].sort(
            (a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()
        );
    })();
    feedItemsRef.current = mergedFeedItems;
    ...
    const buildScreenpipeConfig = useCallback(() => {
        ...
        return {
            ...(s.audioDevice && { audio_device: s.audioDevice }),
            ...(s.outputDevice && { output_device: s.outputDevice }),
            enable_realtime: hasDeepgram,
            ...(hasDeepgram && s.audioDevice && { realtime_audio_device: s.audioDevice }),
    ...
    }, [isLocalWhisper, isDirectDeepgram, localWhisperRunning, directDeepgramRunning, connectionStatus, tauriRunning, buildScreenpipeConfig, settings.deepgramApiKey, settings.audioDevice, settings.outputDevice, settings.localPreferGpu]);
    ...
    const statusLabel = isLoading
        ? pendingActionRef.current === "stopping" ? "Stopping" : "Starting"
        : connectionStatus === "connected"
            ? "Recording"
            : connectionStatus === "checking"
                ? "Connecting"
        ...
        <div className="flex flex-col h-screen bg-background text-foreground noise-bg">
            ...
            <main className="flex-1 min-h-0">
                ...
                >
                    ...
                    >
                        <LiveFeed
                            items={!isLiveFeed ? sessionFeedItems : (isLocalWhisper || isDirectDeepgram) ? localFeed.items : feed.items}
                            isPolling={isLiveFeed && ((isLocalWhisper && localWhisperRunning) || (isDirectDeepgram && directDeepgramRunning) || (!isLocalWhisper && !isDirectDeepgram && feed.isPolling))}
            ...
            <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
                <DialogContent className="w-[96vw] max-w-[1600px] max-h-[92vh] overflow-hidden flex flex-col">
                    <DialogHeader>
