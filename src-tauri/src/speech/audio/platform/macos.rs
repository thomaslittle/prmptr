...
// CPAL is not used for the macOS system track; these exist for the shared
// platform interface and are unreachable for System capture on macOS.
...
fn decode_scalar(bytes: &[u8], bits: u32, is_float: bool, big_endian: bool) -> Result<f32, String> {
    match (bits, is_float) {
        (32, true) => {
            if bytes.len() != 4 {
                return Err("Invalid Float32 PCM sample width".to_string());
            }
        ...
        (64, true) => {
            if bytes.len() != 8 {
                return Err("Invalid Float64 PCM sample width".to_string());
            }
        ...
        (8, false) => Ok((bytes[0] as i8) as f32 / i8::MAX as f32),
        (16, false) => {
        ...
        (24, false) => {
            if bytes.len() != 3 {
                return Err("Invalid Int24 PCM sample width".to_string());
            }
...
fn decode_bytes(bytes: &[u8], bits: u32, is_float: bool, big_endian: bool) -> Result<Vec<f32>, String> {
    ...
    if bytes_per_sample == 0 || bytes.len() % bytes_per_sample != 0 {
        return Err(format!(
            "Invalid PCM byte length {} for {}-bit samples",
            bytes.len(), bits
        ));
    }
    bytes
...
fn decode_sample_buffer(sample: &CMSampleBuffer) -> Result<(Vec<f32>, u32, usize), String> {
    ...
    if !format.is_audio() || !format.is_pcm() {
        return Err(format!(
            "ScreenCaptureKit returned non-PCM audio format: {}",
    ...
    if list.num_buffers() == 1 {
        let buffer = list
            .get(0)
            .ok_or_else(|| "ScreenCaptureKit AudioBufferList is unexpectedly empty".to_string())?;
        if buffer.number_channels as usize != channels {
            return Err(format!(
                "Unexpected interleaved audio layout: format channels={} buffer channels={}",
    ...
    if list.num_buffers() == channels
        && list.iter().all(|buffer| buffer.number_channels == 1)
    {
    ...
    Err(format!(
        "Unsupported ScreenCaptureKit AudioBufferList layout: {} buffers for {} channels",
        list.num_buffers(), channels
    ))
}
...
impl SCStreamOutputTrait for SystemAudioHandler {
    fn did_output_sample_buffer(&self, sample: CMSampleBuffer, output_type: SCStreamOutputType) {
        if output_type != SCStreamOutputType::Audio || !self.running.load(Ordering::Relaxed) {
            return;
        }
        ...
        let (native, rate, channels) = match decode_sample_buffer(&sample) {
            ...
            Err(error) => {
                ...
                log::error!("[system] ScreenCaptureKit audio decode failed: {error}");
                return;
            }
        };
        ...
        let conditioned = match self.conditioner.lock() {
            Ok(mut slot) => {
                let replace = slot
                    .as_ref()
                    .map(|(existing_rate, existing_channels, _)| {
                        *existing_rate != rate || *existing_channels != channels
                    })
                    .unwrap_or(true);
                if replace {
        ...
        let start_sample = self
            .sample_clock
            .fetch_add(conditioned.len() as u64, Ordering::Relaxed);
        match self.tx.try_send(AudioChunk {
            ...
            | Err(mpsc::error::TrySendError::Closed(chunk)) => {
                self.metrics.record_drop(chunk.samples.len());
            }
        }
    }
}
...
pub fn spawn_system_capture_thread(
    ...
    if !supports_system_capture() {
        return Err("ScreenCaptureKit system audio requires macOS 13 or newer".to_string());
    }
    ...
    let thread = std::thread::spawn(move || {
        let content = match SCShareableContent::get() {
            ...
            Err(error) => {
                let _ = ready_tx.send(Err(format!(
                    "ScreenCaptureKit content access failed (check Screen & System Audio Recording permission): {error}"
                )));
                return;
            }
        };
        ...
        let config = SCStreamConfiguration::new()
            ...
            .with_sample_rate(SPEECH_SAMPLE_RATE as i32)
        ...
        let handler = SystemAudioHandler {
            ...
            conditioner: Arc::new(Mutex::new(None)),
            tx,
        ...
        if let Err(error) = stream.start_capture() {
            ...
            let _ = ready_tx.send(Err(format!(
                "Failed to start ScreenCaptureKit system audio: {error}"
            )));
            return;
        }
    ...
    match ready_rx.recv_timeout(std::time::Duration::from_secs(15)) {
        ...
        Err(error) => Err(format!("Timed out starting ScreenCaptureKit system audio: {error}")),
    }
}
...
mod tests {
    ...
    fn signed_integer_pcm_normalizes_to_float() {
        ...
        assert!(decode_scalar(&minimum, 16, false, false).unwrap() <= -1.0);
    }
